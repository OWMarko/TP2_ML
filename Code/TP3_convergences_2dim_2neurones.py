...
for epoch in range(EPOCH):
    ...
    for step, (x, _) in enumerate(train_loader):
        ...
        loss.backward() 
        optimizer.step()
        epoch_loss += loss.item()  # Ajout de la perte

    average_loss = epoch_loss / len(train_loader)  #Moyenne de la perte 
    losses.append(average_loss)  #Stockage 
    print(f'Epoch [{epoch+1}/{EPOCH}], Loss: {average_loss:.4f}')

#Cnvergence de la fonction coût
